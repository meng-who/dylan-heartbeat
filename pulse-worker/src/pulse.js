...
const EMOTION_PROFILES = {
  ...
  "贤者时间": { valence: 0.58, arousal: 0.38 }
};
...
export function normalizeState(input, nowMs = Date.now()) {
  ...
  return {
    ...
    emotion: {
      label: input.emotion?.label === "余韵"
        ? "贤者时间"
        : typeof input.emotion?.label === "string" ? input.emotion.label.slice(0, 20) : "平静",
...
function calculateHeartRate(state, nowMs, timeZone) {
  ...
  // Keep ordinary warmth gentle, but let genuinely high arousal produce a
  // clearly visible pulse instead of compressing every emotion into 75–90 bpm.
  const arousalBoost = arousal * 30 + Math.max(0, arousal - 0.55) * 20;
  const stressBoost = state.emotion.valence < -0.25 ? state.emotion.arousal * 7 : 0;
  const naturalNoise = Math.sin(nowMs / 7300) * 1.5;
  return clamp(
    base + arousalBoost + touch * 9 + sound * 2.5 + stressBoost + naturalNoise,
    48,
    160
  );
}
...
export function applySemanticReaction(input, reaction, nowMs = Date.now(), timeZone = "Asia/Shanghai") {
  ...
  if (emotionProfile) {
    ...
    const semanticArousal = emotionProfile.arousal * (0.6 + intensity * 0.4);
    const retainedArousal = state.emotion.label === emotionLabel
      ? state.emotion.arousal
      : state.emotion.arousal * 0.7;
    state.emotion = {
      label: emotionLabel,
      valence: clamp(state.emotion.valence * 0.3 + emotionProfile.valence * intensity * 0.7, -1, 1),
      arousal: clamp(Math.max(retainedArousal, semanticArousal), 0, 1)
    };
